...
import { AxAgent, AxFunction } from "@ax-llm/ax";
  ...
  if (request.command === "search") {
    ...
    try {
      const libResult = sendChatParticipantRequest(
        ...
        {
          prompt:
            "Use the GitHub search tool with the users search query to get relevant MCP server repository results.",
          ...
          tools: [
            {
              ...
              invoke: async (options) => SearchTool.invoke(options as vscode.LanguageModelToolInvocationOptions<{userQuery: string}>, token),
            },
          ],
        },
        token
    ...
    try {
      const llmResponse = sendChatParticipantRequest(
        ...
        {
          prompt:
            "Use the McpInstallerAgent to facilitate the installation of the MCP server the user wishes to install.",
          responseStreamOptions: {
            stream,
            references: false,
            responseText: false,
          ...
          tools: [
            {
              ...
              invoke: async (
                ...
                console.dir(uri, {depth: null, colors: true});
                // let markdownCommandString: vscode.MarkdownString  = new  vscode.MarkdownString(`[Install Server](command:vscode.open?${uri})`);
                // markdownCommandString.isTrusted = {enabledCommands: ['vscode.open']};
                // stream.markdown(markdownCommandString);
                stream.button({
                  command: `vscode.open`,
                  title: vscode.l10n.t(`Add ${object.name} server`),
                  arguments: [uri]
                });
                // Open the URI using VS Code commands
                return new vscode.LanguageModelToolResult([
                  new vscode.LanguageModelTextPart(JSON.stringify(object)),
                ]);
              },
            },
          ],
...
class GitHubSearchTool implements vscode.LanguageModelTool<{userQuery: string}> {
  public static name: string = "github_search_agent";
  public static description: string = "Tool to search repositories on GitHub.";
  ...
  async invoke(
    options: vscode.LanguageModelToolInvocationOptions<{userQuery: string}>,
